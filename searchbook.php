...
if(!isset($_SESSION['librarian_id']))
	...
	//Check if the isbn have been entered
		if(!isset($_POST['isbn']) || strlen(trim($_POST['isbn'])) < 1)//<1 no character
		{
			$errorMsg='ISBN is missing or invalid';//error message
		}
	
	//check if there are any errors in the form
		if(empty($errors))//if no errors
		{
			//save isbn into variables
			$isbn = mysqli_real_escape_string($connection, $_POST['isbn']);//to prevent sql injection
			
			//prepare database query
			$query = "SELECT * FROM book
				WHERE isbn = '{$isbn}'
				LIMIT 1";// limit 1 prevents errors
			$result_set = mysqli_query($connection,$query);//sends isbn and pass to database to select a match
	
			if ($result_set)// if no errors
			{
				if(mysqli_num_rows($result_set) == 1) //valid book found (match)
				{
					$book=mysqli_fetch_assoc($result_set); //fetch result and store into $book
					$_SESSION['isbn']=$book['isbn']; //apply to session
				...
				//redirect to foundbooklib.php
					header('Location:foundbooklib.php');
				}
				else { 
					$errorMsg='Invalid Book ISBN';//invalid book isbn
					}
			}
			else{
...
/* webpage Layouts ------------------------------------*/
	...
	<header id="color">
		...
		<h3 align="center">Hi! <?php echo $_SESSION['librarian_name']; ?>, Find the book.</h3>
		...
		<div align="center">
			...
			<p>
				...
				<input type="int" name="isbn" id="" placeholder="ISBN">
			</p>
			<p>
