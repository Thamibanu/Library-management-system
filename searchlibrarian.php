...
if(!isset($_SESSION['librarian_id']))
	...
	{
	//Check if the librarian ID has been entered
		if(!isset($_POST['lid']) || strlen(trim($_POST['lid'])) < 1)//<1 no character
		{
			$errorMsg='librarian ID is missing or invalid';//error message
		}
	//check if there are any errors in the form
		if(empty($errors))//if no errors
		{
			//save lid in to variables
			$lid = mysqli_real_escape_string($connection, $_POST['lid']);//to prevent sql injection
			
			//prepare database query
			$query = "SELECT * FROM librarian
				WHERE lid = '{$lid}'
				LIMIT 1";// limit 1 prevents errors
			$result_set = mysqli_query($connection,$query);//sends lid and password to database to select a match
			...
			if ($result_set)// if no errors
			{
				...
				//redirect to foundlibrarian.php
					header('Location:foundlibrarian.php');
				}
				else { 
					$errorMsg='Invalid Librarian ID';//invalid Librarian ID
					}
			}
			else{
...
/* webpage Layouts ------------------------------------*/
	...
	<header id="color">
		...
		<h3 align="center">Hi! <?php echo $_SESSION['librarian_name']; ?>, Find the librarian.</h3>
		<br><br>
		...
		<div align="center">
			...
			<p>
				...
				<input type="int" name="lid" id="" placeholder="Librarian ID">
			</p>
			<p>
